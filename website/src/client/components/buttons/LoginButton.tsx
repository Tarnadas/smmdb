...
class Button extends React.PureComponent<any, any> {
  ...
  public async componentDidMount (): Promise<void> {
    try {
      const response = await fetch(
        resolve(process.env.API_DOMAIN || '', '/login'),
        {
          method: 'POST',
          credentials: 'include'
        }
  ...
  private updateCourseStars (props: any, account: any): void {
    const main: any = {}
    let i = 0
    props.courseData.get('main').forEach(
      (course: any): void => {
    ...
    const main64: any = {}
    i = 0
    props.courseData.get('main64').forEach(
      (course: any): void => {
    ...
    for (let i in account.stars) {
      if (account.stars[i] in main) {
        ...
        this.props.dispatch(
          setCourse(
            courseId,
            this.props.courseData.getIn(['main', courseId]).set('starred', true)
          )
        )
      }
    }
  ...
  private async onGoogleLoginSuccess (accessKey: any): Promise<void> {
    try {
      const response = await fetch(
        resolve(process.env.API_DOMAIN || '', '/login/google'),
        {
          method: 'POST',
      ...
      for (let course of this.props.courseData.get('main')) {
        ...
        if (account.stars.includes(courseId)) {
          this.props.dispatch(
            setCourse(
              i,
              this.props.courseData.getIn(['main', i]).set('starred', false)
            )
          )
        }
        i++
      ...
      for (let course of this.props.courseData.get('main64')) {
        ...
        if (account.stars64.includes(courseId)) {
          this.props.dispatch(
            setCourse64(
              i,
              this.props.courseData.getIn(['main64', i]).set('starred', false)
            )
          )
        }
        i++
  ...
  public render (): JSX.Element {
    ...
    const styles: any = {
      smmButton: {
        ...
        textAlign: 'left',
        fontFamily:
          'SuperMarioMakerExtended,SuperMarioMaker,Roboto,arial,sans-serif'
      },
      google: {
        lineHeight: '40px',
        width: 'auto',
